...
def get_xy_transforms(max_rotate=10., min_zoom=1., max_zoom=2., use_cutout=False, use_noise=False, xtra_tfms=None,
    ...
    if use_cutout:
        x_tfms[0].append(cutout(n_holes=(5, 10)))
    if use_noise:
    ...
    if overlap > 0:
        ...
        for i in range(min(shape[0], shape[0])):
            for j in range(shape[1]):
                if top:
                    mask[i, j] = min((i+1)/overlap, mask[i, j])
                if bottom:
                    mask[h-i-1, j] = min((i+1)/overlap, mask[h-i-1, j])
                if left:
                    mask[i, j] = min((j+1)/overlap, mask[i, j])
                if right:
                    mask[i, w-j-1] = min((j+1)/overlap, mask[i, w-j-1])
...
def unet_image_from_tiles_partialsave(learn, in_img, tile_sz=(256, 256), scale=(4, 4), overlap_pct=(0.50, 0.50), img_info=None):
    ...
    if img_info:
        ...
        mi, ma = 0., 1.
    in_img = np.stack([npzoom(in_img[i], scale, order=1)
    ...
    for i in range(numX+1):
        for j in range(numY+1):
            ...
            if (in_y_size, in_x_size) != src_tile.shape[1:3]:
                set_trace()
            in_tile[0:in_y_size, 0:in_x_size, :] = tensor(
                src_tile).permute(1, 2, 0)

            if n_frames > 1:
                img_in = MultiImage([Image(in_tile[:, :, i][None])
                                    for i in range(n_frames)])
            else:
            ...
            tileROI_x_end = int(
                p-0.5*int(alpha*p)) if crop_x_end != X else int(alpha*p+epsX)
            tileROI_y_start = int(0.5*int(beta*q)) if crop_y_start != 0 else 0
            tileROI_y_end = int(
                q-0.5*int(beta*q)) if crop_y_end != Y else int(beta*q+epsY)
            ...
            out_x_end = int(p-0.5*int(alpha*p)+i*(p-int(alpha*p))
                            ) if crop_x_end != X else X
            out_y_start = int(q-0.5*int(beta*q)+(j-1) *
                              (q-int(beta*q))) if crop_y_start != 0 else 0
            out_y_end = int(q-0.5*int(beta*q)+j*(q-int(beta*q))
                            ) if crop_y_end != Y else Y
            assembled[out_y_start:out_y_end,
                      out_x_start:out_x_end] = out_tile[tileROI_y_start:tileROI_y_end, tileROI_x_start:tileROI_x_end]
...
def unet_image_from_tiles_blend(learn, in_img, use_tiles, tile_sz=256, scale=4, overlap_pct=5.0, img_info=None):
    n_frames = in_img.shape[0]
    if img_info:
        mi, ma, imax, real_max = [img_info[fld]
                                  for fld in ['mi', 'ma', 'img_max', 'real_max']]
        in_img /= real_max
        # in_img = ((in_img - mi) / (ma - mi + 1e-20)).clip(0.,1.)
    else:
    ...
    if not use_tiles:
        tile_sz = h  # assume it is a square
    ...
    for x_tile in range(0, math.ceil(w/step_sz)):
        for y_tile in range(0, math.ceil(h/step_sz)):
            ...
            assembled[trim_y_start:trim_y_end,
                      trim_x_start:trim_x_end] = out_tile[out_y_start:out_y_end, out_x_start:out_x_end]
            if trim_x_start != 0:
                x_seams.add(trim_x_start)
            if trim_y_start != 0:
                y_seams.add(trim_y_start)
...
def tif_predict_images(learn,
    ...
    im.load()
    times = im.n_frames
    if not max_imgs is None:
        times = min(max_imgs, times)
        ...
        for c in channels_bar:
            for z in depths_bar:
                ...
                if (depths > 1) or (channels > 1):
                    pred_out = dest_folder / \
                        f'{czi_in.stem}_c{c:02d}_z{z:02d}_{under_tag}_pred.tif'
                    orig_out = dest_folder / \
                        f'{czi_in.stem}_c{c:02d}_z{z:02d}_{under_tag}_orig.tif'
                else:
                    pred_out = dest_folder / \
                        f'{czi_in.stem}_{under_tag}_pred.tif'
                    orig_out = dest_folder / \
                        f'{czi_in.stem}_{under_tag}_orig.tif'
...
def ensure_folder(fldr, clean=False):
    ...
    if fldr.exists() and clean:
        ...
        shutil.rmtree(fldr)
    if not fldr.exists():
        fldr.mkdir(parents=True, mode=0o775, exist_ok=True)
    return fldr
...
def build_tile_info(data, tile_sz, train_samples, valid_samples, only_categories=None, skip_categories=None):
    if skip_categories == None:
        skip_categories = []
    if only_categories == None:
        only_categories = []
    if only_categories:
        skip_categories = [
            c for c in skip_categories if c not in only_categories]
    ...
    all_files = [fn for fn in list(
        data.glob('**/*.tif')) if is_only(fn) and not is_skip(fn)]
    ...
    tile_df = pd.DataFrame(tile_infos)[
        ['mode', 'category', 'tile_sz', 'h', 'w', 'fn']]
    return tile_df
...
def draw_random_tile(img_data, tile_sz, thresh, thresh_pct):
    ...
    return PIL.Image.fromarray(tile, mode='F'), box
...
def generate_tiles(dest_dir, tile_info, scale=4, crap_dirs=None, crap_func=None):
    ...
    for row_id, tile_stats in progress_bar(list(tile_info.iterrows())):
        ...
        if crap_dirs:
            ...
            crap_tile_folder = ensure_folder(
                crap_dir/mode/category) if crap_dir else None
        else:
        ...
        tile_data.append({'tile_id': row_id, 'category': category,
                         'mode': mode, 'tile_sz': tile_sz, 'box': box, 'fn': fn})
